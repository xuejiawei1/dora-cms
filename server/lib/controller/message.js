...
class Message {
    ...
    async postMessages(req, res, next) {
        ...
        form.parse(req, async (err, fields, files) => {
            try {
                let errMsg = '';
                if (_.isEmpty(req.session.user) && _.isEmpty(req.session.adminUserInfo)&&_.isEmpty(fields.author)) {
                    errMsg = '非法操作，请稍后重试！'
                }
            ...
            if (fields.utype === '1') { // 管理员
                ...
                messageObj.author = !_.isEmpty(req.session.adminUserInfo) ? req.session.user._id : fields.author,
                 ;
            }
